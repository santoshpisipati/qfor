...
{
    public class clsMAWBEntry : CommonFeatures
    {
        string strSql;
        long _Airway_Bill_Pk;

        #region "Properties"

        private Quantum_QFOR.clsTrackAndTrace objTrackNTrace = new Quantum_QFOR.clsTrackAndTrace();
        ...
        public Int16 findJCused_inMJC(string Jcpks)
        {
            ...
            try
            {
                return objWF.ExecuteScaler(strSqlBuilder.ToString());
            }
            catch (OracleException Oraexp)
            {
                throw Oraexp;
        ...
        public string GenerateReference_Number(long Pk, int type, long Airpk, long Locpk)
        {
            try
            {
                ...
                if (Pk > 0)
                {
                    ...
                    if (Strings.InStr(ReferenceNo, "not") > 0)
                    {
        ...
        public int HAWBCount(string MAWBPK = "", string MSJobPK = "", string @from = "MBL")
        {
            try
            {
                ...
                //End If
                totRecords = (Int32)objTotRecCount.ExecuteScaler(strSQL.ToString());
                return totRecords;
            }
            catch (OracleException Oraexp)
            {
