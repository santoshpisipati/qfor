...
{
    public class cls_RFQ_Others : CommonFeatures
    {
        ...
        public DataSet Fetch_surcharge_fortwotable(int Valuepk = 0, string MAIN_TABLE = "", string TRN_TABLE = "", string MAIN_TABLE_PK = "", string PK_OUT = "", string TRN_TABLE_PK = "", int POL_PK = 0, int POD_PK = 0)
        {
            ...
            OracleCommand SCM = default(OracleCommand);
        ...
        public ArrayList SaveSurcharge(ref DataSet dsMain, long PkValue, int TrnPk_colnumber, ref OracleCommand SelectCommand, int Freight_Elepk_colno, string TRN_SEA_PK_COL = "", string TRN_SEA_FREIGHT_COL = "", string MAIN_SEA_COL = "", string TRN_TABLE_IN = "")
        {
            WorkFlow objWF = new WorkFlow();
            arrMessage.Clear();
        ...
        string SURCHARGE_TRN__FREIGHT_FK_COL = "", int POL_MST_FK = 0, int POD_MST_FK = 0)
        {
            ...
            try
            {
                ...
                else if (MAIN_TABLE == "QUOTATION_SEA_TBL")
                {
                    for (RowCnt = 0; RowCnt <= dsGrid.Tables[0].Rows.Count - 1; RowCnt++)
                    {
                        if (!(dsGrid.Tables[1].Columns.Contains("SURCHARGE")))
        ...
        public DataTable FetchRFQHDR(string strPolPk, string strPodPk, string strContId, bool IsLCL, string ValidFrom, string ValidTo, string Mode, int RFQContractPk)
        {
            string str = null;
            WorkFlow objWF = new WorkFlow();
            ...
            if (RFQContractPk > 0)
            {
                ...
                str += "RTS.VALID_FROM, RTS.VALID_TO, RTS.SHIP_TERM_MST_FK";
            ...
            else
            {
                ...
                str += " TO_DATE('" + ValidTo + "','" + M_DateFormat + "') AS \"VALID_TO\", NULL SHIP_TERM_MST_FK ";
        ...
        #endregion "Fetch function for creating New RFQ "
        ...
        public DataTable FetchFreight(string strPolPk, string strPodPk, string strContId, bool IsLCL, string Mode)
        {
            ...
            //'End
            // str &= vbCrLf & " TO_CHAR('0') AS ""CHK"",CURR.CURRENCY_MST_PK,CURR.CURRENCY_ID"
            str += " TO_CHAR('0') AS \"CHK\','";
            str += " NVL(( SELECT T.CURRENCY_MST_FK FROM FREIGHT_CONFIG_TRN_TBL T,SECTOR_MST_TBL SMT WHERE T.FREIGHT_ELEMENT_FK=FMT.FREIGHT_ELEMENT_MST_PK";
            str += " AND SMT.SECTOR_MST_PK=T.SECTOR_MST_FK";
            str += " AND SMT.FROM_PORT_FK=POL.PORT_MST_PK";
            str += " AND SMT.TO_PORT_FK=POD.PORT_MST_PK";
            str += " AND ROWNUM=1),CURR.CURRENCY_MST_PK) AS CURRENCY_MST_PK,";

            str += " NVL((SELECT C.CURRENCY_ID FROM FREIGHT_CONFIG_TRN_TBL T,SECTOR_MST_TBL SMT,CURRENCY_TYPE_MST_TBL C WHERE T.FREIGHT_ELEMENT_FK=FMT.FREIGHT_ELEMENT_MST_PK";
            str += " AND SMT.SECTOR_MST_PK=T.SECTOR_MST_FK";
            str += " AND SMT.FROM_PORT_FK=POL.PORT_MST_PK";
            str += " AND SMT.TO_PORT_FK=POD.PORT_MST_PK";
            str += " AND T.CURRENCY_MST_FK=C.CURRENCY_MST_PK";
            str += " AND ROWNUM=1 ), CURR.CURRENCY_ID) AS CURRENCY_ID";
        ...
        public DataTable ActiveContainers(string strContainer = "")
        {
            ...
            for (i = 0; i <= arrContainer.Length - 1; i++)
            {
                if (string.IsNullOrEmpty(strCondition))
                {
                    ...
                    {
                        strCondition = " ( CMT.CONTAINER_TYPE_MST_ID ='" + arrContainer.GetValue(i) + "')";
                    }
                    else
                    {
                        strCondition = " ( CMT.CONTAINER_TYPE_MST_ID =" + arrContainer.GetValue(i) + ")";
                ...
                {
                    strCondition += " OR ( CMT.CONTAINER_TYPE_MST_ID =" + arrContainer.GetValue(i) + ")";
                }
            }

            if (!string.IsNullOrEmpty(strContainer))
            {
                strSQL = " SELECT Q.CONTAINER_TYPE_MST_PK, Q.CONTAINER_TYPE_MST_ID, Q.CHK   FROM ( SELECT " + " CMT.CONTAINER_TYPE_MST_PK, CMT.CONTAINER_TYPE_MST_ID, " + " '1' CHK ,CMT.PREFERENCES " + " FROM CONTAINER_TYPE_MST_TBL CMT " + " WHERE CMT.ACTIVE_FLAG = 1  " + "AND ( " + strCondition + " ) " + "  UNION " + " SELECT " + " CMT.CONTAINER_TYPE_MST_PK, CMT.CONTAINER_TYPE_MST_ID, " + "(CASE WHEN (" + strCondition + ") " + "THEN '1' ELSE '0' END ) CHK ,CMT.PREFERENCES " + " FROM CONTAINER_TYPE_MST_TBL CMT " + " WHERE CMT.ACTIVE_FLAG=1  " + " ) Q ORDER BY Q.CHK DESC,Q.PREFERENCES";
            }
            else
            {
        ...
        public DataTable ActiveBasis(string strContainer = "")
        {
            ...
            for (i = 0; i <= arrContainer.Length - 1; i++)
            {
                if (string.IsNullOrEmpty(strCondition))
                {
                    if (arrContainer.GetValue(i).ToString() == "0")
                    {
                        strCondition = " ( DUMT.DIMENTION_ID ='" + arrContainer.GetValue(i) + "')";
                    }
                    else
                    {
                        strCondition = " ( DUMT.DIMENTION_ID =" + arrContainer.GetValue(i) + ")";
                ...
                {
                    strCondition += " OR ( DUMT.DIMENTION_ID =" + arrContainer.GetValue(i) + ")";
                }
            }
        ...
        public ArrayList SaveRFQ(ref DataSet dsMain, ref object txtRFQNo, long nPrevRFQPk, long nLocationId, long nEmpId, string txtPOLID = "", string sid = "", string podid = "")
        {
            ...
            try
            {
                ...
                {
                    RFQRefNo = GenerateRFQNo(nLocationId, nEmpId, txtPOLID, M_CREATED_BY_FK, ref objWK, sid, podid);
                ...
                {
                    _with5.Parameters.Add("COMMODITY_MST_FK_IN", DBNull.Value).Direction = ParameterDirection.Input;
                }
                ...
                {
                    _with5.Parameters.Add("BASE_CURRENCY_FK_IN", DBNull.Value).Direction = ParameterDirection.Input;
                }
                ...
                //'
                string CurrFKs = "0";
                DateTime ContractDt = default(DateTime);
                cls_Operator_Contract objContract = new cls_Operator_Contract();
                ContractDt = Convert.ToDateTime(dsMain.Tables["tblMaster"].Rows[0]["RFQ_DATE"]);
                for (int nRowCnt = 0; nRowCnt <= dsMain.Tables["tblTransaction"].Rows.Count - 1; nRowCnt++)
                {
                    CurrFKs += ',' + dsMain.Tables["tblTransaction"].Rows[nRowCnt]["CURRENCY_MST_FK"].ToString();
                }
                objContract.UpdateTempExRate(_PkValue, objWK, CurrFKs, ContractDt, "SLRFQ");
                //'
                if (arrMessage.Count > 0)
                {
                    ...
                    {
                        RollbackProtocolKey("OPERATOR RFQ", Convert.ToInt32(HttpContext.Current.Session["LOGED_IN_LOC_FK"]), Convert.ToInt32(HttpContext.Current.Session["EMP_PK"]), RFQRefNo, DateTime.Now);
        ...
        public ArrayList SaveRFQTRN(DataSet dsMain, long PkValue, OracleCommand SelectCommand)
        {
            ...
            try
            {
                ...
                for (nRowCnt = 0; nRowCnt <= dsMain.Tables["tblTransaction"].Rows.Count - 1; nRowCnt++)
                {
                    ...
                    {
                        _with6.Parameters.Add("CHARGE_BASIS_IN", DBNull.Value).Direction = ParameterDirection.Input;
                    }
                    else
                    {
                    ...
                    _with6.Parameters["SHIP_TERM_MST_PK_IN"].SourceVersion = DataRowVersion.Current;
                    /// END
        ...
        public string GenerateRFQNo(long nLocationId, long nEmployeeId, string POLID, long nCreatedBy, ref WorkFlow objWK, string SID = "", string PODID = "")
        {
            string functionReturnValue = null;
            try
            {
                functionReturnValue = GenerateProtocolKey("OPERATOR RFQ", nLocationId, nEmployeeId, DateTime.Now, "", "", POLID, nCreatedBy, objWK, SID,
                PODID);
        ...
        #endregion "Fetching Port Name and Assigning"
    }
}