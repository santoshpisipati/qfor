...
{
    public class cls_SRRCustToAirListing : CommonFeatures
    {

        #region " Fetch All "

        public DataSet FetchAll(string AirlineID = "", string AirlineName = "", string CustomerID = "", string CustomerName = "", string SRRRefNo = "", string POLID = "", string POLName = "", string PODID = "", string PODName = "", string FromDate = "",string ToDate = "", int STATUS = 0, bool ActiveOnly = true, string SearchType = "", string SortColumn = " AIRLINE_NAME ", Int32 CurrentPage = 0, Int32 TotalPage = 0, string SortType = " DESC ", string CurrBizType = "3", long lngUsrLocFk = 0,
        Int32 flag = 0)
        {
            ...
            if (POLID.Length > 0)
            {
                ...
                buildCondition.Append("         ( SELECT PORT_MST_POL_FK FROM SRR_TRN_AIR_TBL ");
                buildCondition.Append("           WHERE SRR_AIR_FK = main.SRR_AIR_PK ");
            ...
            if (PODID.Length > 0)
            {
                ...
                buildCondition.Append("       and PORT_MST_PK IN ");
                buildCondition.Append("         ( SELECT PORT_MST_POD_FK FROM SRR_TRN_AIR_TBL ");
                buildCondition.Append("           WHERE SRR_AIR_FK = main.SRR_AIR_PK ");
                buildCondition.Append("         ) ");
                buildCondition.Append("     ) ");
            }
            ...
            //End If
            //If ToDate.Length > 0 Then
            //    buildCondition.Append(vbCrLf & " AND main.VALID_TO <= TO_DATE('" & ToDate & "' , '" & dateFormat & "') ")
            //End If
            // APPROVED OR NOT APPROVED
            if (STATUS != 4)
            {
                buildCondition.Append(" AND main.SRR_APPROVED = " + STATUS);
            }
            // ACTIVE ONLY
            //Modified by Snigdharani - 13/11/2008,12/12/2008
            if (ActiveOnly == true)
            ...
            buildQuery.Append("       SRR_AIR_TBL main, ");
            ...
            //buildQuery.Append(vbCrLf & "       decode(main.SRR_APPROVED, 0, 'NA', 1, 'IA',2, 'CA') STATUS ")
            buildQuery.Append("       decode(main.SRR_APPROVED, 0, 'Requested', 1, 'Approved', 2, 'Rejected') STATUS,main.restricted");
            buildQuery.Append("      from ");
            buildQuery.Append("       SRR_AIR_TBL main, ");
            buildQuery.Append("       AIRLINE_MST_TBL, ");
            buildQuery.Append("       CUSTOMER_MST_TBL,USER_MST_TBL UMT ");
            ...
            try
            {
                ...
                DataRelation SRRRel = new DataRelation("SRRRelation", DS.Tables[0].Columns["SRR_AIR_PK"], DS.Tables[1].Columns["SRR_AIR_FK"], true);
        ...
        private string AllMasterPKs(DataSet ds)
        {
            ...
            {
                strBuilder.Append(Convert.ToString(ds.Tables[0].Rows[RowCnt]["SRR_AIR_PK"]).Trim() + ",");
            }
        ...
        private DataTable FetchChildFor(string SRRSpotPKs = "", string POLID = "", string POLName = "", string PODID = "", string PODName = "", string SrOp = "%")
        {
            ...
            {
                buildCondition.Append(" and SRR_AIR_FK in (" + SRRSpotPKs + ") ");
            }
            ...
            buildQuery.Append("      from ");
            buildQuery.Append("       SRR_TRN_AIR_TBL, ");
            ...
            buildQuery.Append("               " + strCondition);
            buildQuery.Append("      Order By SRR_AIR_FK, PORTPOL.PORT_NAME, PORTPOD.PORT_NAME ASC ");
            buildQuery.Append("    ) q ");
            buildQuery.Append("  )   ");
            ...
            try
            {
                ...
                {
                    if (Convert.ToInt32(dt.Rows[RowCnt]["SRR_AIR_FK"]) != pk)
                    {
                        pk = Convert.ToInt32(dt.Rows[RowCnt]["SRR_AIR_FK"]);
                        Rno = 0;
                    }
                    Rno += 1;
        ...
        #region " Enhance Search Function for SRR for CUSTOMER-AIRLINE "

        public string FetchSRRForAirlineCustomer(string strCond)
        {
            ...
            try
            {
                ...
                SCM.CommandText = objWF.MyUserName + ".SRR_REF_NO_PKG.GET_AIRLINE_SRR_COMMON";
